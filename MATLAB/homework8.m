...
disp('Checking the orthogonality propertie between the subtensors formed from the core tensor:')
reshape(tenS_hat(:,:,1),[],1)'*reshape(tenS_hat(:,:,4),[],1)

disp('Checking the NMSE (dB) between the original tensor X and its reconstruction:')
nmsex = (norm(tensor.unfold(tenX - tenX_hat,1),'fro')^2)/(norm(tensor.unfold(tenX,1),'fro')^2);
nmsex = 20*log10(nmsex)
disp('Checking the NMSE (dB) between the original core tensor S and its reconstruction:')
nmses = (norm(tensor.unfold(tenS - tenS_hat,1),'fro')^2)/(norm(tensor.unfold(tenS,1),'fro')^2);
nmses = 20*log10(nmses)

disp('Checking the NMSE (dB) between the original matrices and its estimations:')
...
disp('Checking the NMSE (dB) between the original noisy tensor X and its reconstruction:')
nmsex = (norm(tensor.unfold(X- Xhat,1),'fro')^2)/(norm(tensor.unfold(X,1),'fro')^2);
nmsex = 20*log10(nmsex)
disp('Checking the NMSE (dB) between the original noisy tensor Y and its reconstruction:')
